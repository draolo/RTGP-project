...
void main()
{
   ...
   // FragColor = TurbulenceAAstep();//vec4(color, 1.0f);
    ...
    if(impacted>0){
      vec3 blur=horizontalBlur();
      vec3 red=vec3(1,0,0);
      FragColor = vec4(mix(blur,red,0.),1);//vec4(vec3(0), 1.0);//vec4(color, 1.0f);
    }else{
      //FragColor = TurbulenceAAstep(1);//vec4(color, 1.0f);
      FragColor=texture(screenTexture, texCoords.st);
    }
    //FragColor = vec4(horizontalBlur(),1);//vec4(vec3(0), 1.0);//vec4(color, 1.0f);
